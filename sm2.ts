...
type ResolvedPublicKey = { point: JacobianPoint, publicKey: Uint8Array, table?: JacobianPoint[], windowSize?: number }
const precomputedPublicKeys = new WeakMap<PrecomputedPublicKey, ResolvedPublicKey>()
...
function scalarMultiplyWindow(table: JacobianPoint[], windowSize: number, scalar: bigint): JacobianPoint {
    const digits: number[] = []
    const mask = (1n << BigInt(windowSize)) - 1n
    for (let value = scalar; value > 0n; value >>= BigInt(windowSize)) digits.push(Number(value & mask))
    let result = INF
...
function sqrtMod(value: bigint): bigint {
    let result = modPow(value, (P + 1n) >> 2n, P)
...
function decodePublicKey(publicKey: Uint8Array): AffinePoint {
    ...
    if (x >= P || y >= P || mod(y * y - (x * x * x + A * x + B)) !== 0n) throw new Error('Invalid public key')
    return {x, y}
}
...
function resolvePublicKey(publicKey: PublicKey): ResolvedPublicKey {
    if (publicKey instanceof Uint8Array) {
        ...
        return {point: affineToJacobian(decodePublicKey(canonical)), publicKey: canonical}
    }
...
function multiplyPublicKey(publicKey: ResolvedPublicKey, scalar: bigint): JacobianPoint {
...
function encodePublicKey(point: AffinePoint): Uint8Array {
    return concatBytes(Uint8Array.of(0x04), bigintToBytes(point.x), bigintToBytes(point.y))
}

export function precomputePublicKey(publicKey: Uint8Array, windowSize = 4): PrecomputedPublicKey {
    if (!Number.isInteger(windowSize) || windowSize < 2 || windowSize > 8) throw new Error('Invalid window size')
    const point = affineToJacobian(decodePublicKey(publicKey))
    const table = [INF]
    for (let i = 1; i < 2 ** windowSize; i++) table.push(pointAdd(table[i - 1], point))
    ...
    precomputedPublicKeys.set(prepared, {point, publicKey: canonical, table, windowSize})
    return prepared
}
...
export function generateKeyPair(seed?: Uint8Array): { privateKey: Uint8Array, publicKey: Uint8Array } {
    ...
    return {privateKey: bigintToBytes(privateKey), publicKey: encodePublicKey(point)}
}
...
export function compressPublicKey(publicKey: Uint8Array): Uint8Array {
    ...
    return concatBytes(Uint8Array.of((point.y & 1n) === 0n ? 0x02 : 0x03), bigintToBytes(point.x))
}
...
export function getHash(message: Uint8Array, publicKey: Uint8Array, userId = utf8ToBytes('1234567812345678')): Uint8Array {
    ...
    const parameters = concatBytes(
        entl, uid, hexToBytes(bigintToFixed(A)), hexToBytes(bigintToFixed(B)),
        hexToBytes(bigintToFixed(GX)), hexToBytes(bigintToFixed(GY)),
        hexToBytes(bigintToFixed(point.x)), hexToBytes(bigintToFixed(point.y)),
    )
    return sm3Digest(concatBytes(sm3Digest(parameters), data))
}

function sm2C1(publicPoint: AffinePoint): Uint8Array {
    return concatBytes(hexToBytes(bigintToFixed(publicPoint.x)), hexToBytes(bigintToFixed(publicPoint.y)))
}

function sm2EncryptBytes(message: Uint8Array, publicKey: PublicKey): {
    c1: Uint8Array,
    c2: Uint8Array,
    c3: Uint8Array
} {
...
export function sm2Encrypt(message: Uint8Array, publicKey: PublicKey, cipherMode: SM2Mode = C1C3C2, options: Sm2CipherOptions = {}): Uint8Array {
    if (cipherMode !== C1C2C3 && cipherMode !== C1C3C2) throw new Error('Invalid cipher mode')
    const result = sm2EncryptBytes(copyBytes(message, 'message'), publicKey)
    if (options.asn1) return encodeEncryption(result.c1, result.c2, result.c3, cipherMode)
    return cipherMode === C1C2C3 ? concatBytes(result.c1, result.c2, result.c3) : concatBytes(result.c1, result.c3, result.c2)
}


function parseSm2Cipher(ciphertext: Uint8Array, cipherMode: SM2Mode, asn1: boolean): {
    c1: AffinePoint,
    c2: Uint8Array,
    c3: Uint8Array
} {
    if (asn1) {
        ...
        if (cipherMode === C1C2C3) return {c1, c2: decoded.hash, c3: decoded.cipher}
        return {c1, c2: decoded.cipher, c3: decoded.hash}
    }
    if (ciphertext.length < 96) throw new Error('Invalid ciphertext')
    const c1 = decodePublicKey(concatBytes(Uint8Array.of(0x04), ciphertext.subarray(0, 64)))
    const body = ciphertext.slice(64)
    if (body.length < 32) throw new Error('Invalid ciphertext')
    if (cipherMode === C1C2C3) return {c1, c2: body.slice(0, -32), c3: body.slice(-32)}
    return {c1, c2: body.slice(32), c3: body.slice(0, 32)}
}

export function sm2Decrypt(ciphertext: Uint8Array, privateKey: Uint8Array, cipherMode: SM2Mode = C1C3C2, options: Sm2CipherOptions = {}): Uint8Array {
    if (cipherMode !== C1C2C3 && cipherMode !== C1C3C2) throw new Error('Invalid cipher mode')
    ...
    try {
        ...
        const shared = pointToAffine(scalarMultiply(affineToJacobian(parsed.c1), parsePrivateKey(secret)))
        const x2 = hexToBytes(bigintToFixed(shared.x));
        const y2 = hexToBytes(bigintToFixed(shared.y))
        const message = xorBytes(parsed.c2, kdf(concatBytes(x2, y2), parsed.c2.length))
...
function readDer(input: Uint8Array, offset: number): DerPart {
    ...
    if (firstLength < 0x80) length = firstLength
        ...
        if (count === 0 || count > 4 || offset + 2 + count > input.length) throw new Error('Invalid DER')
...
function decodeDerInteger(part: DerPart): bigint {
    if (part.tag !== 0x02 || part.value.length === 0 || (part.value[0] & 0x80) !== 0) throw new Error('Invalid DER integer')
    if (part.value.length > 1 && part.value[0] === 0 && (part.value[1] & 0x80) === 0) throw new Error('Non-minimal DER integer')
...
function decodeDerValues(input: Uint8Array): { r: bigint, s: bigint } {
    ...
    if (sequence.tag !== 0x30 || sequence.next !== bytes.length) throw new Error('Invalid DER signature')
    const first = readDer(sequence.value, 0);
    const second = readDer(sequence.value, first.next)
    if (second.next !== sequence.value.length) throw new Error('Invalid DER signature')
...
export function encodeDer(signature: Uint8Array): Uint8Array {
    ...
    return encodeDerValues(bytesToBigInt(raw.subarray(0, 32)), bytesToBigInt(raw.subarray(32)))
}
...
function encodeEncryption(c1: Uint8Array, c2: Uint8Array, c3: Uint8Array, cipherMode: SM2Mode): Uint8Array {
    ...
    return derTlv(0x30, concatBytes(x, y, derTlv(0x04, first), derTlv(0x04, second)))
}

function decodeEncryption(input: Uint8Array): { x: bigint, y: bigint, hash: Uint8Array, cipher: Uint8Array } {
    const bytes = copyBytes(input, 'ciphertext')
    const sequence = readDer(bytes, 0)
    if (sequence.tag !== 0x30 || sequence.next !== bytes.length) throw new Error('Invalid ASN.1 ciphertext')
    ...
    if (cipher.next !== sequence.value.length || hash.tag !== 0x04 || cipher.tag !== 0x04) throw new Error('Invalid ASN.1 ciphertext')
    return {x: decodeDerInteger(x), y: decodeDerInteger(y), hash: hash.value, cipher: cipher.value}
}

export function sm2Sign(message: Uint8Array, privateKey: Uint8Array, options: {
    pointPool?: SignaturePoint[], der?: boolean, hash?: boolean, publicKey?: Uint8Array, userId?: Uint8Array
} = {}): Uint8Array {
    ...
    const data = copyBytes(message, 'message')
    const e = hash ? bytesToBigInt(getHash(data, publicKey, options.userId)) : bytesToBigInt(data)
    const inverseOnePlusD = inverse(1n + d, N)
    while (true) {
        const point = options.pointPool?.length ? options.pointPool.pop()! : getPoint()
        const k = bytesToBigInt(copyBytes(point.k, 'point.k'))
        const x1 = bytesToBigInt(copyBytes(point.x1, 'point.x1'))
        if (point.k.length !== 32 || point.x1.length !== 32 || k <= 0n || k >= N || x1 >= P) throw new Error('Invalid signing point')
        const r = modN(e + x1)
...
export function sm2Verify(message: Uint8Array, signature: Uint8Array, publicKey: PublicKey, options: {
    der?: boolean, hash?: boolean, userId?: Uint8Array
} = {}): boolean {
    try {
        const point = resolvePublicKey(publicKey)
        const keyBytes = publicKeyBytes(publicKey)
        const data = copyBytes(message, 'message')
        const e = options.hash === false ? bytesToBigInt(data) : bytesToBigInt(getHash(data, keyBytes, options.userId))
        let r: bigint;
        let s: bigint
